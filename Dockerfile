...
# docker build --tag msd .
# docker run -d -p 8000:8000 -p 8787:8787 --name ms1 msd
# docker run -d -p 8787:8787 --name ms2 msd
...
RUN cd /tmp; curl -O https://cloud.r-project.org/src/base/R-4/R-${R_VERSION}.tar.gz
RUN cd /tmp; tar -zxf R-${R_VERSION}.tar.gz

RUN cd /tmp/R-${R_VERSION}; ./configure --enable-R-shlib --with-blas --with-lapack --with-readline=yes --enable-memory-profiling
